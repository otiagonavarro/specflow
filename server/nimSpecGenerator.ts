...
export type GenerateSpecResult =
  ...
  | {
      ok: false;
      error: 'invalid_input' | 'repo_context_failed' | 'upstream_error' | 'empty_response';
      message?: string;
    };
...
export async function generateSpecFromDescription(input: {
  ...
  try {
    response = await fetch(NVIDIA_CHAT_URL, {
      ...
      body: JSON.stringify({
        model: process.env.NVIDIA_SPEC_MODEL?.trim() || NVIDIA_SPEC_MODEL,
        messages: [
  ...
  if (!response.ok) {
    const detail =
      raw && typeof raw === 'object' && typeof raw.error?.message === 'string'
        ? raw.error.message
        : null;
    let message = detail ?? `NVIDIA API HTTP ${response.status}`;
    if (response.status === 401) {
      message = 'NVIDIA authentication failed (HTTP 401). Check the application API key configuration.';
    } else if (response.status === 403) {
      message =
        detail ??
        'NVIDIA authorization failed (HTTP 403). Check that the model is enabled on your NVIDIA account.';
    }
    return { ok: false, error: 'upstream_error', message };
  }
  ...
  const model = process.env.NVIDIA_SPEC_MODEL?.trim() || NVIDIA_SPEC_MODEL;
